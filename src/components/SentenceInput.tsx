...
const SentenceInput = (props: { stepAtom: PrimitiveAtom<number>, sentenceAtom: PrimitiveAtom<string>, getErrorAtom: PrimitiveAtom<boolean> }) => {
  ...
  return (
    <>
      <label className={`group absolute w-[80%] max-w-7xl block overflow-hidden bg-transparent transition ${step === 0 ? 'z-10' : 'z-20'} -translate-y-[50%]`}>
        <textarea ref={ref} className={` block scroll-smooth overscroll-none resize-none h-[1.75rem] w-full border-none placeholder-silver transition duration-500 ${step === 0 ? 'text-primary dark:text-alabaster opacity-100' : 'text-gray-400 opacity-0'} \
        ...
        <span className={`after:transition-[width] after:block after:absolute after:bg-platinum dark:after:bg-secondary after:h-0.5 after:w-full after:left-0 after:bottom-0 duration-500`}/>
        ...
        <span className={`after:transition-[width] after:block after:absolute after:bg-alabaster dark:after:bg-primary after:h-0.5 after:left-0 after:bottom-0 duration-500 ${step !== 0 ? 'after:w-full after:group-focus-within:w-full' : 'after:w-0'}`}/>
      </label>
    </>
  )
}
